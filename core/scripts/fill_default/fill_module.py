...
from core.scripts.fill_default.fill_data_institution import FillDataInstitution
...
class FillModule:
    ...
    def start(self):
        ...
        for p in Permission.objects.filter(
                codename__in=("view_institutions", "add_institutions", "change_institutions",)):
            module_group_permissions.permissions.add(p)
        ...
        for p in Permission.objects.filter(
                content_type__model=OrderInstitutionQuotas._meta.label.split('.')[1].lower()):
            module_group_permissions.permissions.add(p)
