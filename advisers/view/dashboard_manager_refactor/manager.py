...
class ManagerDashboard:
    ...
    def __init__(self, manager: Managers, number_of_year_max: int = NUMBER_OF_YEARS_DEFAULT):
    ...
    def get_commission_collected_per_range_year(self):
        return self.__get_commission_collected_per_range_year()

    def get_commission_collected_per_range_year_and_adviser(self, adviser_id):
        return self.__get_commission_collected_per_range_year(
            ('adviser_id', adviser_id)
        )

    def get_commission_collected_per_year(self, year):
        return self.__get_commission_collected_per_year(year)

    def get_commission_collected_per_year_and_adviser(self, year, adviser_id):
        self.__query_AND_1.children.append(('adviser_id', adviser_id))
        return self.__get_commission_collected_per_year(year)

    def __get_commission_collected_per_range_year(self, extra_filters: tuple = ()):
        payment_paid_list = []
        self.__set_query_and_clear_to_commission_collected(extra_filters)

        for year in self.get_range_last_year():
            value_commission = self.__get_value_commission_to_year(year, 'commissions_managers_value')
            payment_paid_list.append(self.__get_format_send(year, year, value_commission))

        return payment_paid_list

    def __get_commission_collected_per_year(self, year):
        payment_paid_list = []
        self.__set_query_and_clear_to_commission_collected()

        for mes in MESES:
            value_commission = self.__get_value_commission_to_year_and_month(year, mes[0], 'commissions_managers_value')
    ...
    def get_commission_by_collect_per_range_year(self):
        return self.__get_commission_by_collect_per_range_year()

    def get_commission_by_collect_per_range_year_and_adviser(self, adviser_id):
        self.__query_AND_1.children.append(('adviser_id', adviser_id))
        return self.__get_commission_by_collect_per_range_year()

    def get_commission_by_collect_per_year(self, year):
        return self.__get_commission_by_collect_per_year(year)

    def get_commission_by_collect_per_year_and_adviser(self, year, adviser_id):
        self.__query_AND_1.children.append(('adviser_id', adviser_id))
        return self.__get_commission_by_collect_per_year(year)

    def __get_commission_by_collect_per_year(self, year):
        payment_paid_list = []
        self.__set_query_and_clear_to_commission_by_collect()
    ...
    def __get_commission_by_collect_per_range_year(self):
        payment_paid_list = []
        self.__set_query_and_clear_to_commission_by_collect()
    ...
    def __set_query_and_clear_to_commission_collected(self, extra_filters: tuple = ()):
        self.__clean_query_AND_1()

        self.__query_AND_1.children.append(('deleted', False))
        self.__query_AND_1.children.append(('manager_id', self.__manager.id))
        self.__query_AND_1.children.append(('pay_manager', True))
        for extra_filter in extra_filters:
            self.__query_AND_1.children.append((extra_filter[0], extra_filter[1]))

    def __set_query_and_clear_to_commission_by_collect(self, extra_filters: tuple = ()):
        self.__clean_query_AND_1()

        self.__query_AND_1.children.append(('deleted', False))
        self.__query_AND_1.children.append(('manager_id', self.__manager.id))
        self.__query_AND_1.children.append(('pay_manager', False))
        for extra_filter in extra_filters:
            self.__query_AND_1.children.append((extra_filter[0], extra_filter[1]))
